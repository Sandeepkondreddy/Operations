...
var createStatement = "CREATE TABLE IF NOT EXISTS TransactionsTbl (Id INTEGER PRIMARY KEY AUTOINCREMENT,  TaskCode TEXT, SubTaskCode TEXT, Operation TEXT, Location TEXT, IMEI TEXT, GeoCoordinates TEXT,CreatedTime TEXT,CreatedBy TEXT,Status TEXT)";
 
var selectAllStatement = "SELECT * FROM TransactionsTbl WHERE Status = 'A'";
 
var insertStatement = "INSERT INTO TransactionsTbl (TaskCode, SubTaskCode, Operation, Location, IMEI, GeoCoordinates, CreatedTime, CreatedBy, Status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
 
var updateStatement001 = "UPDATE TransactionsTbl SET username = ?, useremail = ? WHERE id=?";

var updateStatement = "UPDATE TransactionsTbl SET Status = 'I' WHERE id=?";
 
var deleteStatement = "DELETE FROM TransactionsTbl WHERE Id=?";

var deleteAllStatement = "DELETE FROM TransactionsTbl WHERE Status = 'I'";
...
{debugger;
        ...
        db.transaction(function (tx) { tx.executeSql(insertStatement, [TaskCodeTemp, SubTaskCodeTemp, OperationTemp, LocationTemp, IMEITemp, GeoCoordinatesTemp, CreatedTimeTemp, CreatedByTemp, StatusTemp], loadAndReset, onError); });
             ...
             for (var i = 0, item = null; i < dataset.length; i++) {
				...
				//url: 'http://localhost:51594/api/RFIDInternal/OfflineTransDetails',				
						...
						success: function (result) {    
						    ...
						    //deleteRecord(item['Id']);
							deleteOfflineRecords();SaveTaskEndDetails();
						},
						error: function (xhr, status, error) {
  ...
  function deleteOfflineRecords() // Get id of record . Function Call when Delete Button Click..
{
     ...
     db.transaction(function (tx) { tx.executeSql(deleteAllStatement, DeleteSuccessMessage, onError); 
	 //document.getElementById('lblmessage').innerHTML = 'All Offline Data Deleted Successfully.!';});
 }
...
{
     var iddelete = id.toString();
     db.transaction(function (tx) { tx.executeSql(updateStatement, [id], DeleteSuccessMessage, onError); document.getElementById('lblmessage').innerHTML = id+':Trip Deleted Successfully.!';});
 }
